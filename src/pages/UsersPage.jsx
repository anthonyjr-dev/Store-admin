...
// Roles that are not scoped to a branch.
...
function UserModal({ user, branches, onSave, onClose, saving, saveError }) {
  ...
  function update(e) {
    ...
    setForm((f) => {
      ...
      // Super Admin / Admin have no branch.
  ...
  function submit(e) {
    ...
    const payload = {
      ...
      branch_id: branchless || form.branch_id === '' ? null : parseInt(form.branch_id),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl bg-[#1c1c1c] border border-gray-800 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <label className="block text-sm font-semibold text-gray-300">
            ...
            {roleNum === ROLE.ADMIN && (
              <p className="mt-1.5 text-xs font-normal text-gray-500">
                Full order-data access (all branches &amp; kiosk orders) like Super Admin, but the
                portal only shows <span className="text-gray-300">Orders</span>, <span className="text-gray-300">Menu</span>, <span className="text-gray-300">Reports</span> and <span className="text-gray-300">Alerts</span>.
              </p>
            )}
            {roleNum === ROLE.BRANCH_ADMIN && (
              <p className="mt-1.5 text-xs font-normal text-gray-500">
                Scoped to one branch. Portal shows <span className="text-gray-300">Orders</span>, <span className="text-gray-300">Menu</span> and <span className="text-gray-300">Alerts</span>.
              </p>
            )}
          </label>
          ...
          <label className="block text-sm font-semibold text-gray-300">
            Branch Assignment
            <select name="branch_id" value={form.branch_id} onChange={update} disabled={branchless}
